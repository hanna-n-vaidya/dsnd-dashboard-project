...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        # Define the colormap: green -> yellow -> red
        ...
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', pad=0.6, aspect=40)
        cbar.set_label('Risk Level: Low → High', fontsize=9)
        cbar.set_ticks([0, 0.5, 1])
        cbar.set_ticklabels(['0% (Low)', '50%', '100% (High)'])

        # annotating the score
        ax.text(pred + 0.02, 0, f"{pred:.1%}", va='center', fontweight='bold')
